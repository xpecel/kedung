...
@lru_cache(8)
def _user_conf() -> dict[str, dict[str, str | int]]:
    """Menyediakan konfigurasi yg diperlukan agar app bisa berfungsi."""
    r_file: dict[str, dict[str, str | int]]
...
def get_sock_path() -> str:
    ...
    return (
        socket
        if isinstance(socket, str) else
        cast(str, socket.get("socket", default_path))
    )
...
def get_log_path() -> str:
    ...
    return (
        socket
        if isinstance(socket, str) else
        cast(str, socket.get("log", default_path))
    )
...
def get_preallocate_space() -> int:
    ...
    return (
        ...
        cast(int, runtime.get("preallocate_space", default_space))
    )
...
def get_loging_level() -> int:
    ...
    level: str = (
        runtime
        if isinstance(runtime, str) else
        cast(str, runtime.get("logging", default_status))
    )
...
def get_cache_duration() -> int:
    ...
    return (
        duration
        if isinstance(duration, int) else
        cast(int, duration.get("cache_duration", default_space))
    )
